...
用户明确授权“你先修复已有的三个P2”。`codex/day66-p2-fixes`为DentAll 0.41.0/Core 0.2.9，只修改5个既有DentAll主题文件；不新增AJAX端点、匹配算法、模板、插件、字段、数据库迁移或Theme Mod。WooCommerce仍是Variation、价格、库存与服务端加购真相源；本节只关闭RSK-035/037/038的D66/M5 Local门槛，专项分支尚未合入`main`或部署。
...
本终验允许合成树进入`main`，不等于Staging发布批准。D66的RSK-035/037/038已于2026-09-10在DentAll 0.41.0专项分支关闭Local；该分支仍须合入`main`，并在Staging执行代码白名单、文件＋数据库备份、环境差异、配置重放、缓存与回滚预检后重新验证三项。禁止用Local数据库或uploads覆盖Staging既有商品、媒体和订单。
