...
- [ ] 新增、修改数量、删除和空购物车。
- [x] 优惠券成功、失败、过期和同车重复使用（D70隔离Local购物车范围通过；连续长错误展示P2为`RSK-039`，跨订单次数另留D78）。
- [ ] 运费地区、免邮边界和金额舍入。
- [ ] 地址必填、格式错误和错误定位。
- [ ] 支付成功、取消、失败、重复回调和超时。
...
| 权威浏览器/Store API | 脚本SHA-256 `03FD52B92998DE1AB9FFBC0C80054BEEC49B4C00E22DFD3F9DC8A8C6D7CDCE10`；17项通过、1项P2，P0/P1为0；warning 0、预期Console错误11、意外错误0 | 唯一P2在源提交中为`RSK-039`，集成映射为`RSK-041`长券码错误内层裁切；页面无横向滚动，交D68/D72 |
