...
- 当前已形成编号至D70的学习记录，各日证据见下表；D61/D63已随主线集成汇齐，候选学习记录不等于对应Day或里程碑已Done。
- 当前最高编号学习笔记：[[Day70-WooCommerce优惠券校验与金额真相]]（D70交易、恢复与私有环境销毁已收尾；用户掌握度待费曼自测）。
...
D68与D69已分别依据实际候选实现、故障注入和独立Local回放形成学习记录，D70已依据原生优惠券交易与恢复证据形成学习记录；D31～D66既有记录保留原证据范围。D61的RSK-035及D66的RSK-037/038仍未关闭，D68发现的优惠码触控P2仍需D72复验，D69保留两项期限性P2；这些记录不代替W12集成或用户费曼自测。
