...
| 保安登记 | `WC_Rate_Limiter`中的HMAC身份/来源键 | 不是原子锁，也不是WAF |
...
- 输入数据：账号线索、Nonce、服务器看到的`REMOTE_ADDR`；密码仅保留原始字符做长度判断。
...
| HMAC限频键 | 用站点秘密盐把输入映射为不可逆固定标识 | 邮箱/用户名和IP不明文入表 | 普通Hash等于保护隐私 | 查看键形态并确认盐参与 |
...
| `hash_hmac()` | 生成限频ID | 不把原始邮箱、用户名、IP写入表 | 普通Hash可被字典预计算 |
...
| 检查面 | 本次结论 | 证据或待验证项 |
|---|---|---|
| 输入清洗与验证 | 请求方法/字段类型、Nonce、非空、账号归一化、IP格式均检查 | 合同覆盖命名Nonce与`_wpnonce` |
...
| 部署与回滚 | 当前Local候选 | Staging须先验`REMOTE_ADDR` |
...
- 假设症状：所有客户偶尔都收不到重置邮件。
- 可能原因：`REMOTE_ADDR`是共享反向代理地址，来源冷却变成全站冷却。
- 第一项检查：目标PHP实际看到的`REMOTE_ADDR`与代理拓扑。
- 为什么先查它：它会同时解释跨账号、跨浏览器的10秒误伤，且错误信任转发头会引入伪造风险。
