...
状态: 已收口
掌握度: 初识
验证环境: Local；WordPress 7.0.4、WooCommerce 11.0.0、Storefront 4.6.2、DentAll 0.5.0、PHP 8.2.9
tags:
  - DentAll
  - WordPress实战
  - CSS级联
...
- 掌握：子主题CSS作用域、低权重`:where()`、状态优先级、Focus与错误状态、真实页面回归。
...
flowchart TD
    ...
    C --> D["DentAll子主题style.css?ver=0.5.0"]
    D --> E["浏览器匹配作用域、状态和级联"]
...
源文件：`style.css`。以下真实片段展示“正常Hover必须排除禁用状态”：
...
):where(:hover:not(:disabled):not([aria-disabled="true"]):not(.disabled)) {
	border-color: var(--dentall-color-action-hover);
	background-color: var(--dentall-color-action-hover);
}
```

源文件：`style.css`。以下真实片段展示按视觉表面分区的键盘Focus，以及DOM位于Footer、视觉却属于深色Header表面的手机固定栏例外：

```css
.site .site-content :where(a:any-link, button, input, select, textarea, summary):focus-visible,
.site .site-footer :where(a:any-link, button, input, select, textarea, summary):focus-visible {
	outline: 3px solid var(--dentall-color-focus-on-light);
	outline-offset: 3px;
}

.site .site-header :where(a:any-link, button, input, select, textarea, summary):focus-visible,
.site .site-footer .storefront-handheld-footer-bar :where(
	a:any-link, button, input, select, textarea, summary
):focus-visible {
	outline: 3px solid var(--dentall-color-focus-on-dark);
	outline-offset: 3px;
}
```
...
| 缓存 | 主题版本更新为0.5.0 | 仍为原单个CSS请求；只更新资源缓存键 |
| 性能 | CSS为14604字节，无新请求、查询或脚本 | 未做前后性能量测，不宣称零影响 |
...
| CSS改了但页面旧 | 版本、缓存或错误主题 | URL版本→HTTP响应→Computed | 确认`style.css?ver=0.5.0` |
...
- DentAll 0.5.0在Storefront子主题`style.css`中用`.site-main`、真实壳层作用域、`:where()`和Design Token建立展示基线；WooCommerce继续负责真实控件、Variation、排序和状态类。
