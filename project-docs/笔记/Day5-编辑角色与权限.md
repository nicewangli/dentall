...
- WordPress权限应按业务动作设计，而不是直接套用角色名称；原生角色往往比真实岗位拥有更多能力。
- 隐藏后台菜单只改善界面，不构成安全控制。真正的权限必须同时阻止直接URL、保存动作、REST/AJAX入口和对象级越权。
- 最小权限不是一次性冻结，而是从白名单起步，经过真实任务验证后按具体动作逐项扩权。
- 权限测试应使用目标角色账号覆盖允许与拒绝两类用例；管理员账号测试通过不能证明编辑角色安全可用。
- 部署分支可以把“日常开发状态”和“Staging可部署状态”分开；只读Deploy Key减少托管平台反向修改仓库的风险。
- 测试数据需要在验收后清理。能创建、能限制、能清理，才是一条完整的后台验证链路。
