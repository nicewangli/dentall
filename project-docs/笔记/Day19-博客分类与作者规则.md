...
- 后台责任：两名Website Manager继续使用各自独立账号直接创建、编辑、审核和发布；`post_author`保留内容作者，受支持字段的修订版本记录对应修改账号和时间，不在发布前改派给额外品牌账号。发布、撤回和回收站等状态动作不能从这两类记录反推，转D23另行治理。
...
- REST抽查两篇已发布TEST文章：路由均为`/blog/{slug}/`，Yoast输出`Article`结构化数据，后台`post_author`分别保留实际账号，证明统一公开署名规则没有改派内容作者。该证据不证明发布或其他状态动作人。当前Yoast仍把实际账号输出为`Person`作者，因此前台`DentAll Editorial Team`署名与对应Schema尚未实现；该差距登记为D87文章详情模板/SEO输出验收项，不在D19通过修改账号显示名、增加品牌账号或改第三方插件核心文件处理。
...
- 本次没有新增品牌作者账号或角色，也没有把两名Website Manager的后台显示名改成相同名称；现有`post_author`继续保留内容作者，受支持字段的修订版本继续保留对应修改账号。
...
| 作者与内容修订 | 两名Website Manager使用各自账号直接写作和发布，后台`post_author`保留内容作者，受支持字段修订记录对应修改账号；第一版不新增账号或角色 | 两篇已发布TEST文章保留不同内容作者；既有权限允许Website Manager发布，但D19证据不构成发布状态操作者审计 | 通过；状态动作留痕转D23，前台统一署名与作者Schema转D87实现 |
...
- 统一署名由后续前台模板与SEO输出实现，不新增用户登录面；实现时不得覆盖后台内容作者和受支持字段的修订记录。
...
- 跨平台不变量：登录账号、内容作者、字段修改人、审核人、状态操作人和前台公开署名是不同职责。把它们强行合并会泄露内部身份、制造虚假审计或让公开署名失真。
- WordPress/WooCommerce当前实现：独立用户账号负责权限，`post_author`负责内容作者归属，原生修订追踪受支持字段的修改版本，Yoast控制作者归档与结构化数据；发布/撤回等状态动作需要额外留痕。单一品牌署名站点通常不需要重复的作者归档，但仍应在文章页清楚显示真实内容责任方。
