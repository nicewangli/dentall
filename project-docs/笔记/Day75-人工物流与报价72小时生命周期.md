...
周次: W13
实际有效工时: 用户未记录
验收层级: 隔离Local技术验收与Staging首次签发/单一Action通过；真实网关竞态留D76/D78
状态: Local完成；Staging首次签发与单一到期Action已通过
---

# Day75 人工物流与报价72小时生命周期

> 批次①集成补充（2026-09-22）：首封Customer Invoice现在会在发送前校验正数Shipping与完整地址合同；资料不完整时不发送、不签发、不调度。Shipping服务端白名单为美国、加拿大、澳大利亚；Billing国家不限制并按WooCommerce国家地址字段规则判断州省/邮编必填性。已签发Guest报价因D79邮箱验证归户而改变Customer ID时按既有签名规则取消，Website Manager建立替换报价。原D75源验收为83/83；批次①新增邮件门禁、归户协作和后台提示后为89/89。
...
- 生命周期模块已随DentAll Core 0.4.1部署到Staging；发布候选为`f81f236`，部署前文件＋数据库恢复点为`2026-09-22 03:11:13 UTC`。本轮生命周期纯合同复验89/89，仓库PHP lint、Node语法与`git diff --check`通过。
- PayPal尚未完成配置，Store API可用支付方式为空，因此当前不能验证真实网关开始支付后的晚到回调；这与D76/D78边界一致。
- 当日WooCommerce日志没有Fatal、Error、Critical、Warning、Deprecated或Uncaught记录。经用户明确授权，建立TEST订单`#1579`：不完整首封被SMTP前门禁阻断，补齐美国地址和`$12`正数Shipping后，Customer Invoice于`2026-09-22 05:58:12 UTC`成功发送并首次签发。
- 订单系统备注确认截止时间为`2026-09-25 05:58:12 UTC`，即首次成功发送后72小时；Action Scheduler搜索`dentall_expire_shipping_quote`只返回1项待执行动作，参数为`order_id => 1579`，计划时间与截止时间一致。没有为测试重发邮件，因此“不续期”继续由89/89合同与Local真实重发证据覆盖。
- 当前Staging已经关闭“不完整首封阻断、完整首封签发、单一Action”三项代表路径。目标主机到期后的真实自动取消/实时拒付、实质字段变化、邮箱验证归户替换单和PayPal晚到webhook仍分别等待截止时间或D76/D78；不能据此标记真实支付闭环或M6完成。
- Cloudways Password Protection按用户决定暂不启用；该风险与生命周期正确性分别记录，不用noindex替代访问控制，也不向外部人员分发测试付款链接。
