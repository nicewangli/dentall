...
- 更新日期：2026-08-17。
- 当前阶段：W3 / D14已完成Local可变商品边界、默认值、父/变体SKU、价格、独立库存、缺货、非法组合、物流继承/覆盖和购物车流程；父子重复库存真相源P1已修复。下一步进入D15库存与物流字段。
...
- D15：复核重量、尺寸、测量对象、物流单位和Variation继承/覆盖规则，并验证库存管理和停售候选流程。
...
1. D15区分产品规格、单个销售包装物流数据和最终订单外箱，确认字段测量对象及来源。
2. 验证Variable物流相同时继承父值、实际不同时由Variation覆盖，并复核当前`lbs/in`是否符合后续承运商方向。
3. 验证库存状态、临时缺货与停售候选规则，继续保持真实订单、支付、税费和正式物流配置关闭。
