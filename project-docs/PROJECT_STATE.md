...
- D85已按用户确认的最小范围在独立Local副本完成：原生Page继续使用现有Full width模板，主题候选`0.43.0`只增加普通Page正文阅读行长与连续字符换行两块局部CSS；22个页面场景616/616、Website Manager Gutenberg编辑回归67/67、静态检查和独立代码审查均通过。三个明确TEST Page、两个临时用户及编辑标记已清理，隔离服务已停止；共享Local、Staging、菜单、SEO配置、订单、支付和物流均未写入。详见[[笔记/Day85-通用内容页模板与编辑回归]]。

- 更新日期：2026-09-21。
- 当前阶段：既有D67～D72与D66修复已进入`main`且尚未部署；本工作树的D85仅完成隔离Local技术验证，未提交、合并、推送或部署。D85不补记D73～D84，也不改变W15/M7及其他Day状态；正式Page内容仍由业务方协调后验收。
...
- 当前版本：`main`与`origin/main`基线为`e9b5d2b`，DentAll 0.42.0/Core 0.3.0；本工作树的D85候选仅把DentAll升至0.43.0，尚未提交。Staging上次登记的部署基线为`501e5e5`/Core 0.2.6，远端`deploy/staging`仍为该提交；本轮没有读取或修改服务器文件、插件版本或哈希，不能据此排除现场漂移。共享Local和任何非Local配置均未因D85改变。
...
- 2026-09-21用户明确授权D85仅限隔离Local：采用原生Page、现有Full width模板和既有Token，允许明确TEST代表样本；只有实测缺口才补最小正文CSS及必要加载调整，并完成四端、键盘、编辑、SEO和文档回归。授权明确排除共享Local、Staging、菜单、SEO配置、其他Day、提交、合并、推送和部署。
- 隔离副本创建短、长、空三种TEST Page，均为原生Page、Published、Full width，并由隔离Website Manager持有。长页复用既有媒体，覆盖标题、段落、列表、引用、图片、表格、长文本和连续标识符；未上传媒体、未创建自定义字段、模板、插件或正式内容。
- 基线实测证明1024/1440px正文阅读行分别约115/151字符，390px表格外层可视宽350px而滚动宽584px，确有阅读和裁切缺口。最终只在既有`style.css`内增加两块作用于`.page-template-template-fullwidth:not(.woocommerce-page)`的规则：文章最大宽度46rem并居中，正文`min-width: 0`与`overflow-wrap: anywhere`；主题版本升至0.43.0用于资源缓存失效。没有新增CSS请求、条件加载、函数、模板或DOM。
- 最终22个页面场景共616/616断言通过：三类Page在390/768/1024/1440px均无页面横向溢出，文章宽度为350/704/736/736px，图片尺寸/替代文本/边界正常，长表格不再溢出；Home、Shop、Product、Cart和My Account在390/1440px均未命中D85选择器。实际键盘验证覆盖Skip Link进入主内容、正文链接、可见3px焦点轮廓和Shift+Tab返回；空页因没有正文链接按不适用记录。
- SEO回归确认每页单一Title、H1、main和article，隔离环境继续输出单一`noindex, nofollow, noarchive`且不输出Canonical，JSON-LD可解析并保持WebPage URL；未修改URL、Slug、Reading、Yoast、菜单或Sitemap配置。全站noindex环境不能代替公开环境自身Canonical和索引验证。
- Website Manager以非管理员角色在Gutenberg完成67/67断言：3个区块加载，预览/自动保存标记只出现在预览，正式保存后前台可见，再通过核心编辑器精确恢复；最终正文哈希、标题、作者、状态、Slug和Full width模板不变，修订数在最终一轮由4增至6，编辑器无脏状态。Reading、Yoast、导航和菜单哈希均未改变。
- 运行差分仅1个既有文件：13行新增、1行删除、净增12行；新增运行文件0、函数0、规则块2、CSS声明4、主题版本元数据1项、资源请求0。PHP lint、CSS花括号104/104、`!important` 0、行内风险0及`git diff --check`通过；独立代码审查P0/P1/P2/P3均为0。
- 三个TEST Page、两个临时用户、全部编辑标记和自动保存已清理；订单/退款保持0，WooCommerce session保持源基线同一条记录。隔离PHP/MySQL端口已关闭。最终只读复核确认共享Local的`wp-config.php`哈希、Git HEAD/状态、选项、商品、文章、元数据和terms与复制前一致。
- 剩余边界：正式Page文案与素材、真实设备/屏幕阅读器、公开环境Canonical、Staging/Production、缓存/CDN及复杂固定宽度嵌入仍待对应阶段验证；若首页未来改用Full width模板或正式内容出现复杂表格，应重新回归选择器作用域与内容溢出。
...
- D62收尾时其他Day尚待；D59～D61与D63其后分别完成。本轮将D60/D61/D63合成，下一检查点为D66，正式内容与非Local验收仍待。

## D63并行检查点：原生扩展信息与公开资料空状态

- 2026-09-07用户明确批准：“第一版只面向匿名公开的补充资料，当前无合格PDF时不输出、不开放PDF权限，并以零运行代码/零数据库变更完成原生展示与缺字段验收。”实施严格限于只读验证、文档收口、独立复核和范围提交。
- 参数继续由可见商品级属性承载，跨商品筛选只使用已批准Global Attributes，影响价格、SKU、库存或合法组合的选项仍由Variations承担；少量经批准认证与说明文字放Description。没有新增字段、ACF定义或重复事实源。
- WooCommerce 11.0.0源码确认：Description仅在正文非空时注册；Additional Information仅在存在可见属性或需展示的重量/尺寸时注册，`WC_Product::has_attributes()`会检查`get_visible()`。当前自有主题/Core没有改写Tabs、属性或维度过滤器。
- 当前#44 Simple的原生附加信息为Weight、Dimensions、Package Quantity；#46 Variable为Weight、Dimensions、Size、Shade。D65主分支合成证据中的8个390/768/1024/1440视口均无页面横向溢出、坏图且H1为1；重复ID只在#44的390px及#46四宽记录为0。#46单一路径以键盘激活Additional information后焦点为3px实线且面板可见，Console为空；没有把它写成四宽逐一Tab交互。当前23个自有运行文件与D65隔离副本统一换行后逐文件比对为23/23相同，`ad6f26e`只改文档，因此该证据代表D63运行基线；D63重新解析保存HTML，没有启动或创建新的数据库副本、测试商品、媒体或持久测试状态。
- 两个代表商品的Description/Additional Information面板内均无PDF或资料链接，Variation数据的`is_downloadable`均为false，也没有正式认证；无合格文件时页面没有资料入口或空下载容器。这是正确空状态，不代表PDF上传、公开文件正向下载、失效撤回或购买后授权下载已验收。
- 独立权限复核确认Content Editor仅允许JPEG/PNG/WebP，Website Manager另允许CSV；两者均拒绝PDF、无`unfiltered_upload`且上限为5,242,880字节。匿名无Cookie访问现有uploads图片的`HEAD`为200，证明普通媒体URL是公开空间，页面密码、隐藏链接或`noindex`不能构成文件访问控制。
- 理论P3：异常丢失的属性taxonomy或第三方Filter在Tab已注册后清空最终属性数组，可能留下空面板；当前代码与两件TEST商品没有该条件，不为假设边缘新增守卫。Woo/Storefront或相关插件升级、taxonomy异常或出现实页空面板时复验。
- D63运行层净增文件、函数、规则块、字段、查询、前端资源和行数均为0；未执行商品、媒体、角色、ACF、设置、URL/SEO、缓存配置、支付、物流、订单或部署写入，数据库业务/配置显式写入为0。D65的25表哈希只证明其隔离验证边界；D63本轮没有当前数据库全表前后哈希，不能据此断言每个完整HTTP请求都不会触发session、Cron、transient或第三方插件写入。主题与Core版本不升。
- 本日最高验收层级为“当前Local运行基线的原生承载与空状态技术收口”。正式参数、认证、PDF授权、长值实页、失效文件、实体设备/屏幕阅读器、Staging/Production和公开缓存撤回未验；该历史验证不代替D66集成回归。详见[[笔记/Day63-原生扩展信息与公开资料空状态]]。
...
- [x] 用户明确授权仅在Local实施推荐最小范围：复用WooCommerce原生商品信息，只修改现有详情样式，移除Storefront重复品牌缩略图并保留`product_meta`文字品牌，更新主题版本；禁止修改商品/评价数据及新增字段、模板、JS、插件或购买逻辑。
- [x] DentAll 0.32.0在既有`product-detail.css`中为标题、评分、Regular/Sale/Variation价格、短描述、库存和Meta建立Token化层级；Woo原生Sale标签绝对定位到Gallery左上12px，#44摘要不再被推低约43px。金额、币种、库存、评价和字段条件仍由Woo输出。
- [x] `dentall_remove_storefront_product_brand_thumbnail()`于`after_setup_theme` 40只移除`woocommerce_single_product_summary`上的Storefront缩略图回调@4；`woocommerce_product_meta_end`仍保留`WC_Brands::show_brand@10`，`woocommerce_structured_data_product` Brands Filter、taxonomy、筛选、归档和数据不变。
- [x] #44 Simple在六宽保留`$29.99 → $24.99`、Sale、8 in stock、SKU、分类及原/现价辅助文字；#46 Variable保留`$39.99–$49.99`、父SKU、2个Select和3个Variation。两商品0评分/0品牌时相应容器不输出，没有用CSS或TEST数据伪造正向内容。
- [x] 390/768/1024/1199/1200/1440均无横向溢出；Sale在Gallery内且不碰灯箱入口/Summary，768～1440双列顶线一致，1199/1200列宽按D55合同切换。Product加载1份`product-detail.css?ver=0.32.0`；Home/Shop为0，页面Console error/warning为0。
- [x] 只读审计确认#44/#46及Variation 51～53价格、库存、SKU、状态、分类、品牌和评价不变；Product URL、Canonical、H1和Schema保持。PHP lint、CSS 221行/7170字节/32对花括号/0 `!important`、`git diff --check`通过。探索性只读WP-CLI因Windows引号/受保护方法/错误端口写入工具Fatal和连接警告但未改数据；02:29:29 UTC后页面验收未新增前台PHP错误，历史日志未清空。
- [x] 独立测试发现Woo 11的`screen-reader-text`位于促销`del`与`ins`之间，使`del + ins`成为死规则；已改为局部`del ~ ins`并经Code复核关闭P2。Code/范围审查另关闭评分clearfix Flex间隙、库存无收益Flex、泛化命名及失真注释；最终三路复核P0=P1=P2=P3=0。
- [x] 减法审查删除6条重复/无收益声明；最终4个既有运行文件122行新增/2行删除、净+120行，新增0运行文件、1函数、1 Action、16个CSS规则块；0模板/JS/插件/字段/查询/远程请求/数据写入。六宽为DOM/几何证据而非六档独立截图，最终促销间距仅补拍390px；正向评分/品牌实页、RTL、真实辅助技术、D59/D61与非Local仍为明确边界。

## D58 简单商品购买区与隔离购物车验证

- [x] 用户明确授权Day58最小范围、独立Local验证副本及仅在副本进行可逆TEST商品状态和购物车测试；发现标签名称P2后，又明确授权最小展示Filter、最终复验和提交。
- [x] DentAll 0.33.0继续使用WooCommerce经典Simple POST表单、原生quantity、Form Handler、Cart、库存判断和notice；`product-detail.css`只增加购买表单、可见数量与标签的4个局部规则，`woocommerce_quantity_input_args`只为当前Simple主商品清空展示用`product_name`，由Woo模板输出可翻译的`Quantity`。
- [x] Simple在390/768/1024/1199/1200/1440下横溢出0，输入/按钮44px、标签与输入间距8px；可见`Quantity`被输入可访问名称`Product quantity`包含。数量2可用Tab/Enter提交，成功notice为`role=alert`并获取焦点，商品库存仍8。
- [x] 独立HTTP完成16项矩阵：正常、0/空/负数/小数/超限、购物车累计上限、重复POST、缺货、空价格、stock1、独售、catalog hidden和Draft均按实际Woo合同通过。重复成功POST会累计数量，未实现或宣称防重、幂等、AJAX或稳定pending按钮。
- [x] 副本使用独立文件、唯一数据库、专用数据库账号、回环地址、新salt、noindex及Cron/邮件/WP HTTP/支付/Checkout/XMLRPC阻断；源库SELECT返回1142。15个匿名cart已清空，#44/#46/#51/#52/#53由新进程完整比对恢复，库存8、独售false、订单/退款0且不变；共享Local及其原有购物车未触碰。
...
## D59 商品详情四端购买区与Sticky收口

- [x] 用户明确批准推荐最小范围、隔离Local可逆测试及关闭共享Local的Storefront原生Sticky；不授权替代固定购买栏、D61 Variation动态行为、插件/模板/JavaScript或非Local配置。
- [x] DentAll 0.34.0在同一Woo经典单品DOM上增加一个`48rem`～`74.999rem`局部规则：Gallery/Summary取消float、同为内容区满宽并清除逻辑尾边距；390px原单列不变，1200px起继续D55约57%/39%双列，Tabs保持在两者之后。
- [x] Gallery平板`sizes`改为`calc(100vw - 4rem)`并保留移动/PC公式；五图缩略图允许390px收缩、从768px起每列封顶100px。部分lazy副图由WordPress合法增加`auto, `前缀，公式本体保持一致。
- [x] 共享Local的Theme Mod从“键缺失、继承Storefront默认true”保存为显式false，其他Theme Mod逐项不变；隔离实页六宽均为0 Sticky DOM/0脚本，1440推荐图顶部命中图片自身，D64/D65约91px遮挡P3关闭。配置不会随Git传播，非Local未改。
- [x] 权威隔离浏览器矩阵：正常Simple/Variable六宽247/247、多图六宽144/144、缺图四宽80/80、长文本四宽88/88；页面横溢出、Console/Page/Request failure均0。Simple标签/44px和Variable初始form/2 Select、D64推荐1/2/3/3列与48px尾距、D65 Product/BreadcrumbList唯一性均回归。
- [x] 多图/缺图/长文本只写隔离副本；#44/#46/51～53及modified时间恢复，`products_equal/orders_equal/refunds_equal=true`且订单/退款仍0。首轮`ABSPATH`重复定义warning属于夹具缺陷，修正后全部权威`*-final`证据重跑，旧attempt不作终态。权威汇总后一次只读`wp eval`因Windows引号转换产生工具侧Fatal但无写入，随后用无内联PHP命令读回0.34.0与Sticky=false；隔离PHP/MySQL及15959/15960均已关闭。
- [x] 减法审查：3个既有运行文件12行新增/8行删除、净+4行；新增0运行文件/函数/Hook/模板/JS/插件/字段/查询。独立Code Review P0=P1=P2=P3=0；真实设备/辅助技术、RTL实页、D61动态Variation、正式内容、CWV和非Local仍待。

## D60 商品详情代表内容回归与W10收口

- [x] 用户明确授权“同意按功能确认单实施 D60，业务来源素材仅限隔离 Local 内部验证”；两组业务来源材料只作为带TEST边界的长英文/五图Simple与Variable初始展示样本，不登记为批准公开或正式商品事实，不写共享Local。
- [x] 独立文件/数据库副本仅监听`127.0.0.1:16060/16061`，源库SELECT为1142拒绝；副本noindex并阻断外部HTTP、邮件、Cron、Action Scheduler、支付、Checkout和XML-RPC。测试结束服务与端口均关闭。
- [x] 六宽390/768/1024/1199/1200/1440覆盖业务Simple/Variable初始、缺图、售罄和无价格不可购买，共30页/30截图；原始401/419经Woo 11源码与多维DOM证据复核，18项均为Oracle假阴性，权威419/419。Console/Page Error均0；17条响应式WEBP候选`ERR_ABORTED`对应图片均完成解码。
- [x] Variable六宽保持两个Select未选、`variation_id=0`和原生未选类，不选择或加购；原生按钮无HTML `disabled`属性的语义强化仍留D61。无价格Simple六宽购买表单0、Product Schema 0、BreadcrumbList 1且无悬空引用，符合Woo在无Offer/Rating/Review时的原生分支。
- [x] Simple仅执行一次数量2加购并得到成功notice/购物车数量2；D64当前推荐区最多3项、1/2/3/3列与D65有价商品Product/Breadcrumb唯一性新鲜回归，D64空态/三项完整矩阵复用未变的既有证据。Home/Shop详情CSS与Gallery均0。
- [x] #44/#46/#51～#53完整Woo CRUD数据及modified相等，订单/退款0、pending actions 14、原session键/值/到期哈希保持；新增session精确删除。10个附件、10个上传原图、69个派生WEBP、10个夹具副本、临时用户和凭据均清理，递归测试前缀计数0。
...
- [x] 用户明确授权“按上述 D64 最小范围实施，包含隔离 Local 验证及可逆 TEST 样本。”D64提前完成后已与D58/D62/D65集成；当时不替代D59～D61或D63验收，D59～D61其后已分别完成，D63也已按零扩展原生展示范围完成。
...
- [x] 四端1/2/3/3，`mixed-browser.json`11行通过、errors为空、图片完成解码；相邻间距P2经一条局部margin修复后独立四宽均48px/overflow 0。原生推荐回调冷/暖查询基线与D64均32/0，不代表整页/CWV/大目录性能。D64当时发现的1440px Sticky部分遮图P3经D58/D65集成复现，后于D59关闭Local原生配置并回归；未单独回放D57，不推断历史归因。
...
- 本次修复源提交`5c4cefb`由`codex/day66-p2-fixes`承载，已通过非快进合并纳入并推送`main`，修复分支保留且临时工作树已清理；尚未部署。共享Local业务数据、商品/Variation、媒体、订单和非Local配置未因该结论改变。详细范围见[[笔记/Day66-商品浏览闭环集成回归]]。
- 项目笔记与独立学习笔记、索引及直接前置笔记已显式互链；学习掌握度由用户本人自测，不以自动生成标记掌握。

## D68阶段结果：响应式Cart Block候选已完成隔离技术验证

- 用户明确授权D68最小范围，并允许在全新独立Local副本创建、恢复TEST购物车及验证Page ID 8英文内容；源Local全程只读。运行候选继承D67重放提交`21f2941`，只修改3个既有主题文件：`cart.css`、其既有条件加载说明注释与主题版本；相对D67为65行新增/35行删除、净增30行，新增运行文件、函数、Hook、模板、JavaScript、查询、插件和依赖均为0。
- 独立终审汇总198/198断言通过，覆盖390/768/1024/1199/1200/1440px、Simple与Variation #51、数量/金额Store API合同、长名称/连续属性/缺图、loading/update error/售罄/不可购买/96字符错误、空态、Cross-sell图片、键盘/焦点/中心命中及清车。实际只加载一次`cart.css?ver=0.37.0`，数量选择器128×44px，增减与Remove均44×44px，Checkout为48px。
- Page ID 8只在隔离库用WordPress原生API验证`Cart`及三段批准英文；ID 8、slug `cart`、URL `/cart/`、publish、Woo绑定与Block拓扑不变。候选内容哈希验证后已恢复中文原哈希；源Local只读事务快照与`wp-config.php`哈希不变。隔离数据库、商品/Variation与购物车均恢复，16868/16869监听为0。
- 独立功能发现仅1项P2：原生`Add coupons` disclosure六宽均20px，虽Enter及三点指针可正常展开，仍由D70统一完成44px触控目标；负责人D70，延期原因是优惠码入口属于D70而非D68。真实设备/读屏器、公开环境Canonical、CDN/缓存/CWV及Staging/Production未验；不标D68/D67/M5 Done、不合并或部署。

## D69并行检查点：Header Cart与Mini Cart状态联动候选

- 2026-09-08用户明确同意按D69最小范围实施。当前只连接Cart Block的公开`wc/store/cart`与既有`wc-cart-fragments`，不重写Cart/Mini Cart，不新增接口、第二Store、持久状态、轮询、交易规则或非Local配置。
- 修改前隔离基线证明：Cart Store 1→2时Header仍为1，刷新后才为2；同页移除Store为0时Header仍为2。实施后Simple和Variable均完成1→2→0同页同步，匿名/Customer、非空/空Mini Cart、鼠标/键盘/模拟触屏、390/768/1024/1440及资源条件通过。
- 桥接先完成Store resolver，再按商品key/quantity签名订阅；在resolver前已用公开jQuery AJAX生命周期跟踪localized精确fragment端点。Woo首屏/跨标签/BFCache请求与桥接进入同一批次，全部settle后按内存revision有界收敛；Store失败不误刷，fragment失败或缺目标fragment不自动重试，下一次真实变化恢复。
...
- 隔离源`public`复制范围为13,421个文件，复制前源、复制后源与目标副本树SHA-256同为`A9C8759A6F6335AC20D74F6FE79E7991274F1DC33F8D0921EDBB832B4CDEF528`；自定义主题17文件、Core 7文件另行校验。WordPress 7.0.4、WooCommerce 11.0.0、PHP 8.2.29、Storefront 4.6.2、DentAll 0.35.0/Core 0.2.8，HPOS启用。
- 权威浏览器脚本SHA-256为`03FD52B92998DE1AB9FFBC0C80054BEEC49B4C00E22DFD3F9DC8A8C6D7CDCE10`，17通过/1 P2，P0/P1为0，warning 0、预期Console错误11、意外错误0；浏览器于`2026-09-08T06:07:15.381Z`结束，后配置审计于`06:07:41Z`得到105/105，与前审计105/105一致。15张券、订单0、用量0、`free_shipping=false`与角色表保持不变。
- 三种原生券型、Simple＋Variable混合适用、重复/过期/未知、min/max数量重算、individual-use两顺序、匿名隔离、Customer当前购物车、网络失败重试与失败后服务端状态不变通过。独立结果SHA-256为`21F06E5F0E31E011D5C20231CCF9786861AC5640B5AA208258FE0C96DB66A0CF`，8/8且P0/P1/P2为0；`fixed_product`实际每件$2.25。过期短暂骨架约5秒后恢复，新Oracle为骨架0、真实商品#44在、行/总额均2499。
- 唯一P2在D70源提交中编号为`RSK-039`，集成时映射为`RSK-041`：96字符连续券码错误当时在内层裁切、四端页面无横向滚动；该项随后由D68最小展示规则和D72/W12六宽回归在Local关闭，留Staging复验。D71税费、D75免邮/运费、D78跨订单次数仍是独立验收；D70当日没有Checkout、订单、支付、库存或邮件。
...
- 主题切换前后快照`stylesheet/template/current_theme`并通过WordPress主题API/WP-CLI验证结果，精确保存和恢复两个`theme_mods_*`、`sidebars_widgets`、Custom CSS及location映射；`theme_switched`/`theme_switched_via_customizer`仅作诊断。任何WordPress引导前先经只读数据库入口确认两项无真值，否则本轮`NO-GO`；需要归一化时另开已授权且先备份的主题状态修复窗口。获准主题切换后，才由实际加载目标主题和正常插件集、不带`--skip-themes`/`--skip-plugins`的受保护请求完成生命周期；绝不回灌旧真值，目标主题Fatal时转回滚。WordPress会自动映射菜单，因此`secondary`必须未绑定、`handheld`必须未注册、前台只保留一棵主导航，不能在发布窗口创建/删除/改名菜单项处理映射。
...
- 激活DentAll会写入活动主题相关数据库选项并触发菜单自动映射；同一`dentall` stylesheet slug还可能重新启用旧0.1.0残留的Theme Mod或Custom CSS。不先保存option存在状态、完整值、Custom CSS和菜单对象/映射就切换，可能复活旧样式或产生Secondary第二导航。应用备份、现场快照和短暂写入冻结是本轮硬闸门。
...
- **状态：Day65主分支Local集成范围已完成。** D58、D62、D64与D65合成后重跑12页Schema/DOM；商品与合法Variation URL均为Product 1、BreadcrumbList 1、悬空引用0，非商品页无回归。#44四端无横向溢出，D64推荐网格为1/2/3/3列；Simple数量2加购、notice、购物车金额与清空链路通过。D59～D61已另行完成；D63其后完成原生空态收口，D66仍待集成回归。
- 当时D65合成树P0/P1/P2未发现；D65记录的Sticky局部遮图P3已由D59关闭Local原生配置并回归。未来替换原生Schema生成器仍需兼容复验；正式内容、Google/Schema在线验证、屏幕阅读器、真实设备、生产缓存与CWV未验。
