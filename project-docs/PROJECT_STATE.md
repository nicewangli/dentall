...
- 更新日期：2026-08-17。
- 当前阶段：W3 / D13已完成Website Manager培训者引导预演、不到5分钟的无提示复跑，以及简单商品发布字段、促销、库存、Backorders、缺图和回收站恢复验证；未发现P0/P1。下一步进入D14可变商品边界与Variation流程。
- 当前计划：单休20周编辑先行版，120个工作日，自然周期约4.6个月，对外按4.5～5个月管理。
- 当前里程碑：M1技术预验收已在D6通过，Website Manager培训者预演已在D13通过；两名实际网站工作人员仍需在到岗后分别补30分钟无指导试录，完成M1人员验收。该人员项不阻塞D14骨架推进；D18商品模型候选冻结，D24内容样本完成，D25开放批量录入。
...
- D10 C5已形成重量与尺寸规则v1候选：产品规格、单个可售单位发货数据与最终订单外箱分开；Simple维护自身物流数据，Variation可继承父值或按实际覆盖；只读确认Local当前为重量`lbs`、尺寸`in`，仍需结合物流方案在D15前复核，未修改WooCommerce配置。
...
- D13人员验收补项：两名实际网站工作人员到岗后，分别使用独立Website Manager账号完成30分钟无指导试录；培训者预演不能替代该结论，但不阻塞D14骨架。
- D14：使用D12保留的TEST Variable父商品及3个合法Variations，冻结可变商品边界、默认值、父/变体SKU和库存流程，并验证无效组合。
...
1. D14确认哪些购买选项必须成为Variation，哪些只作为展示属性，避免把理论组合全部生成。
2. 复核Variable父商品与每个可售Variation的唯一SKU、有效价格、库存真相源、默认选择和物流继承/覆盖。
3. 验证3个合法组合可购买、未建立的非法组合不可选，并将字段与操作步骤标记为D18前候选。
