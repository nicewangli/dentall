...
1. 从最终`main`生成受控Staging代码发布白名单；`deploy/staging@501e5e5`与`main`历史和目录布局不同，不直接合并。
2. 写入Staging前完成文件＋数据库备份、环境差异、配置重放、缓存和回滚预检；禁止用Local数据库或uploads覆盖Staging既有商品、媒体和订单，部署后在Staging重新验证RSK-035/037/038。
