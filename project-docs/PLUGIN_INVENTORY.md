...
|---|---|---|---|---|---|---|---|
| DentAll Core | 项目业务 | 跨主题角色、权限及后续商城业务规则 | Local与Staging已安装并激活 | 0.1.2 | GPL/项目自有 | 中 | 停用后保留角色数据；回滚代码并重新分配账号角色 |
...
| ACF Pro | 字段 | 仅在原生字段不足时补充定制商品展示、技术参数和资料下载等结构化字段 | 已选型，待公司购买Personal许可证；商品资料到达前不建立最终字段组 | 待安装时记录 | 商业许可证/公司账户 | 中 | 字段定义通过Local JSON或PHP版本化；停用前评估模板依赖和数据迁移 |
| Yoast SEO Free | SEO | SEO标题、Meta描述、Canonical、XML Sitemap、基础Schema和编辑辅助 | 已选型，待Local安装与权限验证 | 待安装时记录 | GPL/免费 | 中 | 导出设置后替换；禁止与其他SEO插件重复输出元数据和Schema |
