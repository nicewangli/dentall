...
### D80客户密码重置（2026-09-22，Staging邮件链路验收完成）

- 将WooCommerce My Account作为第一版统一找回入口；WordPress核心`lostpassword`与`retrievepassword`请求跳转到Woo入口，既有`rp/resetpass`有效链接不被拦截。
- 对存在与不存在账号返回同一标题、说明和URL；60秒身份冷却只保存HMAC标识，不保存邮箱或用户名明文，命中后不刷新期限。目标代理链未证明`REMOTE_ADDR`是客户端地址，因此来源级限频留给可信边缘层，避免共享代理误伤全站。
- 继续复用WordPress/WooCommerce原生重置密钥、邮件、有效期、单次使用和成功后登录，只增加新密码至少12字符的服务端规则；未增加CAPTCHA、独立重发按钮或第三方安全插件。
- DentAll升至0.45.0、DentAll Core升至0.5.0。D80合同28/28、D79回归16/16、五类页面状态四宽浏览器审计135/135通过，页面错误、控制台错误和请求失败均为0；独立代码、安全和测试复核P0/P1=0。
- 隔离Local使用3名`example.test` TEST客户和私有邮件捕获，没有外发邮件；客户、限频记录、隔离数据库、运行目录和监听均已清理。Staging经备份后部署`a07af22`，只向D77受控邮箱发送一次；公开通用确认、FluentSMTP成功记录、Woo事务邮件日志、手机收件与有效链接进入新密码表单均通过，账户页继续noindex且`private/no-cache`。未提交新密码、未新增账户或发送第二封，Production未部署。
