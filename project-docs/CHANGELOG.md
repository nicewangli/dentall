...
- 对存在与不存在账号返回同一标题、说明和URL；60秒身份冷却与10秒直连来源冷却只保存HMAC标识，不保存邮箱、用户名或IP明文。命中任一冷却后不刷新期限、不写入其他限频项。
...
- 隔离Local使用3名`example.test` TEST客户和私有邮件捕获，没有外发邮件；客户、限频记录、隔离数据库、运行目录和监听均已清理。Staging真实投递、FluentSMTP私有日志、代理地址合同和缓存绕过仍是发布验收项，Production未部署。
