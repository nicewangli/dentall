...
### D80客户密码重置（2026-09-22，Local候选完成）
...
- 隔离Local使用3名`example.test` TEST客户和私有邮件捕获，没有外发邮件；客户、限频记录、隔离数据库、运行目录和监听均已清理。Staging真实投递、FluentSMTP私有日志和缓存绕过仍是发布验收项，Production未部署。
