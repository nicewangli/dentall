...
### D62补充：保留选型方向，当前不引入字段

- 2026-09-07用户明确批准当前代表商品零新增字段、零运行代码、ACF继续停用及文档增量提交。原生属性、Variations和描述足以承载当前已验证需求，不能由候选清单反推必须启用ACF。
- Local实物为免费版ACF 6.8.7；活动插件不含ACF、定义计数0、Pro代码不存在。纠正“Pro已安装”的当前事实，不据此追溯判断D16安装历史或公司许可证；采用Pro前仍需重新核验账户与环境条件。
- CR-005及R-015保持已确认业务方案、待实施；D58～D61与D63不在本次范围。未来新增字段、PDF权限或插件仍需独立功能确认、安全与恢复验证。
- 详细证据和文档回滚见[[笔记/Day62-原生字段复核与零扩展收口]]。

### D63补充：第一版只承载匿名公开资料，无合格文件则不输出

- 2026-09-07用户明确批准D63采用匿名公开补充资料模型，并以零运行代码、零数据库变更完成原生展示和缺字段验收。参数使用可见商品级属性，少量经批准认证与说明使用Description；Global Attributes与Variations继续按筛选和交易职责决定。
- 当前没有通过授权、安全、版本与体积门槛的PDF，因此不显示下载入口、不开放Website Manager PDF权限、不启用ACF或新字段。收到文件、能够上传和允许公开是三项独立事实。
- 普通WordPress媒体URL按公开空间管理；页面密码、隐藏链接和`noindex`不作为文件访问控制。购买后受控下载与内部证据文件明确不属于本范围，未来出现真实需求时重新确认订单资格、存储、次数/期限、退款撤权、缓存与恢复。
- D63只形成技术承载与正确空状态结论，不代表正式参数、认证、资料内容、公开下载正向路径或M5完成。详见[[笔记/Day63-原生扩展信息与公开资料空状态]]。
...
## ADR-037：D70复用WooCommerce原生优惠券与Store API金额真相

- 状态：已接受（2026-09-08）；用户明确批准三种原生券型及TEST矩阵、隔离Local写入与恢复，不加插件、不自定义金额、不扩权。
- 决定：第一版优先使用WooCommerce `percent`、`fixed_cart`和`fixed_product`原生规则；以`WC_Coupon`配置、`WC_Cart`/`WC_Discounts`与Store API的服务端整数金额为交易真相，Cart Block只负责输入与展示。不新增计算器、组合规则、插件或主题/Core交易代码。
- 数据与权限：15张`TEST-D70-*`券、1个TEST Customer和1个边界商品只在独立Local存在；全部券`free_shipping=false`，没有Checkout或订单。Website Manager既有优惠券能力与Content Editor拒绝合同保持，角色表不变；HPOS保持启用。
- 验证证据：配置前/后审计均105/105，浏览器/Store API权威轮次17项通过、1项P2，P0/P1为0；独立交易复核8/8且P0/P1/P2为0。唯一P2 `RSK-039`是96字符连续券码错误的内层裁切，交D68候选处理、D72回归，不在D70跨职责改CSS。
...
- 状态：用户已明确回复“按上述 D64 最小范围实施，包含隔离 Local 验证及可逆 TEST 样本。”独立Local最小代码、动态验证、业务字段恢复/TEST清理、最终独立复核及服务关闭已完成，专项增量已纳入`main`；本记录不占用并行任务可能冲突的下一全局ADR编号。
...
- 合并与回滚：D57继承快照`6ece0ce`与D64提交分开，主任务已仅合入D64专项差异并统一版本；回退D64代码无需删除正式关联数据。D59/D61、D66和非Local部署边界保持，详见[[笔记/Day64-原生关联商品与推荐空状态]]。

## D61决定：原生Variation事实与最小展示适配

- 状态：用户于2026-09-08明确同意既有Day61 v1范围，前提D59/D60核验正常；两项前置已复核。实施只在D60基线任务分支与新隔离Local进行，不移动共享main或部署。
- WooCommerce继续负责属性、默认值、匹配、价格库存、不可购买及POST加购。子主题复用数量标签Filter和D59尺寸公式，新增一个`woocommerce_available_variation` Filter、五块CSS及一个720字节的条件ARIA脚本；不新增模板、字段、插件、AJAX接口或交易状态机。
- 独立JS有独立的浏览器加载生命周期，因此单文件；其余职责沿用现有主题资源与展示Hook，不新建Core模块。净运行代码+108行、+1文件、+2 PHP函数、+1 Filter；资源仅在Variable单品额外请求，版本升0.35.0，Core不变。
- URL/SEO实现和真实商品数据不改，原生缓存/服务端交易校验保留；不宣称性能零影响。回滚只需逆向D61主题增量并恢复版本，不存在数据迁移；测试数据与服务恢复单独验收。
- 已知Woo原生AJAX在已有可购选择后网络失败时可保留旧class/展示，按UX P2留项；开发者在D66复审，当前不把class语义映射扩成未授权的loading/error状态系统。详见[[笔记/Day61-原生变体选择与购买验证]]。
